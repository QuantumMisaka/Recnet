...
from . import constraints as constraint_utils


def generate_initial_adsorbate_guesses(ctx: WorkflowContext):
    """For each species, place it on every unique site, relax, and rank by energy."""
    slab = ctx.slab
    ...
    def _opt(
        ...
        if constraints_in is None:
            constraints = ctx._build_constraints(
                struct,
    ...
    for sp_id, data in ctx.ads_templates.items():
        ...
        for site in sites:
            ...
            if ctx.enable_rotation_enum_ads:
                stru, best_angle, best_energy = _enumerate_azimuth_orientation(
                    ctx, base_stru=stru,
                    ads_idx=data["ad_idx"],
                    myslab=myslab,
                    pos=pos,
            ...
            if e_site is not None:
                ...
                if os.path.exists(opt_xyz):
                    ...
                    if not intact:
                        ...
                        close_ok, height = _is_adsorbate_close_to_surface(
                            ctx, opt_atoms, sp_id, pos, max_height=2,
                        )
                        if not close_ok:
                            print(
                            ...
                            rescue_site_params, rescue_center_params = ctx._build_site_and_center_anchors(
                                data, pos, len(myslab),
                            )
                            ...
                            rescue_atoms.set_constraint(
                                ctx._build_constraints(rescue_atoms, surf_atom_num=len(myslab),
                            ...
                            if e_site is not None:
                                ...
                                close_ok2, height2 = _is_adsorbate_close_to_surface(
                                    ctx, rescue_atoms, sp_id, pos, max_height=2.2,
                                )
            ...
            for site in sites:
                ...
                try:
                    ...
                    atoms.set_constraint(
                        ctx._build_constraints(
                            atoms, surf_atom_num=len(myslab), with_internal_bonds=False,
                        )
                    )
        ...
        if ctx.top_x is not None and ctx.top_x > 0:
            valid_sites_preferred = valid_sites_all[:ctx.top_x]
        else:
...
def _is_adsorbate_close_to_surface(ctx, atoms, sp_id, site_pos, max_height=2.2):
